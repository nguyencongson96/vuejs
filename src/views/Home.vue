...
async function handleClick() {
    ...
    try {
        const res = await api.get('/drink/match', {
            params: {
        ...
        router.push({
            path: `/match/${result.drink._id}`,
            query: { genreId: selectGenre._id, flavorId: selectFlavor._id },
        })
    } catch (error) {
...
onBeforeMount(async () => {
    const res = await Promise.all([api.get('/genre'), api.get('/flavor')])
    ...
    <div id="wrapper" class="container-fluid text-light d-flex align-items-center main">
        <div class="carousel-container m-auto d-flex align-items-stretch justify-content-center gap-2">
            ...
            <div class="text-container d-flex flex-column align-items-center gap-3 position-relative text-center">
                ...
                <button
                    :ref="isNotMatched"
                    class="mt-3 btn btn-primary fs-3 rounded-pill fst-italic px-4 py-3"
                    @click="handleClick"
                >
                    Surprise me !!!
                </button>
    ...
    <v-dialog v-model="isNotMatched" max-width="540">
        <template>
            <v-card>
                ...
                <v-card-text> Your combination is unavailable. Please try other combo. </v-card-text>
            </v-card>
        </template>
    </v-dialog>
</template>
